...
public class OutputView {
    private static final String OUTPUT_QUANTITY_MESSAGE = "수동으로 %d장, 자동으로 %d개를 구매했습니다.%n";
    ...
    public static void outputLotteryNumbers(LotteryGame lotteryGame) {
        int quantity = lotteryGame.getQuantity();
        int numberOfAutomatic = lotteryGame.getNumberOfAutomatic();
        int numberOfManual = lotteryGame.getNumberOfManual();
        System.out.printf(OUTPUT_QUANTITY_MESSAGE, numberOfManual, numberOfAutomatic);
        for (int i = 0; i < quantity; i++) {
            System.out.println(lotteryGame.getLotteries().get(i).getNumbers());
        }
        System.out.println();
    }
