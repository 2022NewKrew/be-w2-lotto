...
public class LottoGame {
    ...
    private static List<Lotto> divideManualLottosStr(String manualLottosStr) {
        ...
        if (manualLottosStr.length() > 0) {
            insertManualLottos(manualLottos, manualLottosStr);
        }

        return manualLottos;
    }

    private static void insertManualLottos(List<Lotto> manualLottos, String manualLottosStr) {
        List<String> manualLottosStrs = Arrays.stream(manualLottosStr.split("\r?\n"))
                .collect(Collectors.toList());

        for (String manualLottoStr : manualLottosStrs) {
            manualLottos.add(new Lotto(manualLottoStr));
        }
    }
